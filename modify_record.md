...
## [2026-07-07 23:44] v1.23.6 — 聲紋 GPU/DirectML (Vulkan) 支援 + 自動 fallback CPU

- **version**: 1.23.5 → 1.23.6 (patch: 修 vulkan/dml GPU 加速聲紋)
- **修改要求**: 使用者反饋「vulkan GPU 在聲紋辨識 無法使用gpu，就算 enable vulkan 選項」。v1.23.5 雖然 UI 上有勾選「useGpu」checkbox，但程式碼從未把 useGpu flag 傳遞給 voiceprint module，導致 GPU 加速永遠是 hardcoded CPU，勾選無效。
- **修改規劃**:
  1. **voiceprint.js (核心修正)**: loadModel(modelKey, useGpu = false) 改為接受 useGpu 參數，根據 useGpu 決定 EP 陣列 useGpu ? [dml, cpu] : [cpu]，優先嘗試 DirectML (DML) — onnxruntime-node 1.27.0 在 Windows 11 + WDDM 2.9+ 上會自動透過 Vulkan API 轉譯 GPU 運算。若 DML 失敗 (例如 v1.20.12 記錄的 campplus AveragePool 拋 80070057)，自動 fallback 到 CPU 路徑，避免整個 diarize 失敗。新增 _lastLoadProvider / _lastLoadUseGpu 私有狀態變數（移至 loadModel 之前），cache hit 判斷包含 useGpu 比較。新增 getCurrentProvider() 對外讀取當前 session 實際使用的 EP (dml / cpu)。
  2. **cascade 到所有 voiceprint functions**: setActiveModel(modelKey, useGpu)、_ensureModelLoaded(modelKey, useGpu)、diarizeAudio(...useGpu)、propagateSpeakers({useGpu})、identifySpeakers({useGpu})、buildProfile(...useGpu)、buildProfileFromAudioFile(...useGpu) 全部加 useGpu 參數並轉傳。
  3. **module.exports**: 新增 getCurrentProvider 匯出。
  4. **main.js IPC handler 修正**: 7 個 voiceprint:* handler 加 useGpu：voiceprint:status (回傳 provider 欄位)、voiceprint:download (下載後 setActiveModel 也帶 useGpu)、voiceprint:diarize / voiceprint:propagate (opts.useGpu) / voiceprint:identifySpeakers / voiceprint:backfillAll / voiceprint:jobSubmit (新 useGpu 欄位透傳)。
  5. **VoiceprintJobManager**: addJob({useGpu}) 與 _executeJob 接受 useGpu 並傳給 voiceprint.diarizeAudio。
  6. **App.vue 5 個呼叫點**: voiceprintDownload / voiceprintJobSubmit / voiceprintPropagate / voiceprintIdentifySpeakers / voiceprintBackfillAll 都帶 useGpu: this.useGpu。preload.js 不用改 — payload pass-through 已正確。
  7. **升級 package.json**: 1.23.5 → 1.23.6。
- **修改結果**:
  - voiceprint 模組現在支援 ONNX DirectML (Vulkan) EP，失敗自動 fallback CPU
  - 5 個 voiceprint 操作都會傳遞 useGpu flag
  - voiceprint:status IPC 回傳新增 provider 欄位
  - voiceprintJobManager 新增 useGpu 欄位，背景 diarize job 也會走 GPU 加速
  - 修改日誌即將同步
- **備份檔名**: 即將產生 (backup-202607072344.zip)
